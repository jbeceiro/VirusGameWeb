...
export default function SinglePlayerGameScreen({ user, onGameEnd }: Props) {
  const { ui, myId, isMyTurn, startGame, reset, selectCard, playSelectedCard, onTransplantTarget, toggleDiscardMode, toggleCardForDiscard, confirmDiscard, dismissTargetDialog, clearError } = useSinglePlayer(user)
  ...
  useEffect(() => { startGame(); return () => reset() }, [])
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-b from-[#1A1A2E] via-green-900/70 to-green-900 overflow-hidden relative">
      ...
      <div className="flex gap-2 overflow-x-auto px-2 py-1">
        {otherPlayers.map(pid => {
          ...
          const highlighted = new Set(
            selectedCard?.type === 'VIRUS' && myTurn ? Object.entries(pState.body).filter(([, s]) => getOrganState(s) !== 'IMMUNIZED' && colorsCompatible(selectedCard.color, s.organCard.color)).map(([k]) => k)
            : selectedCard?.treatmentType === 'ORGAN_THIEF' && myTurn ? Object.entries(pState.body).filter(([, s]) => getOrganState(s) !== 'IMMUNIZED').map(([k]) => k)
            : selectedCard?.treatmentType === 'TRANSPLANT' && myTurn ? Object.entries(pState.body).filter(([, s]) => getOrganState(s) !== 'IMMUNIZED').map(([k]) => k)
            : []
          )
          ...
          return (
            <PlayerBodyView key={pid} playerState={pState} playerName={playerNames[pid] ?? 'IA'} isCurrentTurn={gs.playerOrder[gs.currentPlayerIndex] === pid} isMe={false} highlightedColorKeys={highlighted} firstTargetColorKey={firstTargetKey} onOrganClick={onOrganClick} onPlayerClick={onPlayerClick} />
          )
        })}
      </div>
        ...
        <div className="px-2 mb-2">
          {(() => {
            const myHighlighted = new Set(
              selectedCard?.type === 'MEDICINE' && myTurn ? Object.entries(myState.body).filter(([, s]) => getOrganState(s) !== 'IMMUNIZED' && colorsCompatible(selectedCard.color, s.organCard.color)).map(([k]) => k)
              : selectedCard?.type === 'ORGAN' && selectedCard.color === 'MULTICOLOR' && myTurn ? ['RED', 'YELLOW', 'BLUE', 'GREEN'].filter(c => !myState.body[c])
              : selectedCard?.treatmentType === 'TRANSPLANT' && myTurn ? Object.entries(myState.body).filter(([, s]) => getOrganState(s) !== 'IMMUNIZED').map(([k]) => k)
              : []
            )
            ...
            return <PlayerBodyView playerState={myState} playerName="Tú" isCurrentTurn={gs.playerOrder[gs.currentPlayerIndex] === myId} isMe={true} highlightedColorKeys={myHighlighted} firstTargetColorKey={myFirstKey} onOrganClick={onMyOrganClick} />
          })()}
        </div>
      )}
      ...
      {error && <div className="bg-red-900/80 text-red-200 text-sm text-center py-1 px-4">{error}</div>}
